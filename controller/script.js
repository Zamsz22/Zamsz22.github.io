...
const API_KEY = "hf_aEDiXAdxEHQttGJdBqlTowbgeTTILqsD"; // ¡CUIDADO! No exponer en producción.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (wizardPrompt) {
        const mainPromptInput = document.querySelector('.prompt-input'); // Este es el textarea de tu imagen
        if (mainPromptInput) {
            mainPromptInput.value = wizardPrompt; // ¡Aquí se establece el valor del textarea!
            console.log("Prompt cargado desde el asistente:", wizardPrompt); // Para depuración
        }
        localStorage.removeItem('wizardPrompt'); // Importante: Limpiar para no usarlo de nuevo en la siguiente recarga
        
        // Opcional: Si quieres que el cursor aparezca al final del texto para edición inmediata
        if (mainPromptInput) {
            mainPromptInput.focus(); // Da el foco al textarea
            // Mover el cursor al final del texto (esto es un poco más elaborado pero útil)
            // mainPromptInput.setSelectionRange(mainPromptInput.value.length, mainPromptInput.value.length);
        }
    }
});